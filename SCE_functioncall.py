...
def RunMod(npar,x,model):
    Qv = np.array([0, 0.164, 0.544, 0.249, 0.147, 0.281, 0.545, \
          0.629, 0.337, 0.719, 0.139, 0.389, 0.372, 0.444, 0.5, 0.141, 1.134])
    x[0]=0.5

    con = model.predict(np.array([np.array(x[i]) for i in range(17)]).reshape(-1,17))
    #con = model.predict(xx)
    obj= -sum([Qv[i]*x[i] for i in range(17)])
#    R = [1.38,1.27,1.76,3.15]
    xx = np.sum(x)
    for i in range(0,4):
#        if(con[0,i] > 1.5/R[i]):
        if(con[0,i] > 1.5):
...
def EvalObjF(npar,x,model,testcase=True,testnr=1):
    '''
