...
def cceua(s,sf,bl,bu,icall,maxn,iseed,model,testcase=True,testnr=1):
...
##    fnew = functn(nopt,snew);
    fnew = EvalObjF(nopt,snew,model,testcase=testcase,testnr=testnr)
    icall += 1
...
def sceua(x0,bl,bu,maxn,kstop,pcento,peps,ngs,iseed,iniflg,model,testcase=True,testnr=1):
...
#                10 shuffling loops
    ...
    xf=np.zeros(npt)
    for i in range (npt):
        xf[i] = EvalObjF(nopt,x[i,:],model,testcase=testcase,testnr=testnr)
        icall += 1
    f0=xf[0]
...
##                        idx=find(lcs(1:k3-1)==lpos)
                ...
                snew,fnew,icall=cceua(s,sf,bl,bu,icall,maxn,iseed,model,testcase=testcase,testnr=testnr)
    ...
    #reshape BESTX
    len = int(BESTX.size/nopt)
    BESTX=BESTX.reshape(len,nopt)
